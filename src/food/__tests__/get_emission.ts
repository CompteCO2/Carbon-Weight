...
import { ComsumptionR, ConsumptionT } from "../types";
...
const runner = (dataset:DataI[]) => {
  dataset.forEach((data:DataI) => {
    test(data.testDescription + "- Emission", () =>
      expect(Math.floor(getEmission(data.consumption).emission)).toBeCloseTo(data.expectedResult.emission)
    );
    test(data.testDescription + "- Waste", () =>
      expect(Math.floor(getEmission(data.consumption).waste)).toBeCloseTo(data.expectedResult.waste)
    );
  })
}
...
const WRONG_DATA = [
  {
    consumption : {
      alcohol: 24, bread: 2, cheese: 1, fish: 1, harvestExotic: 1,
      harvestLocal: 1, meatRed: 1, meatWhite: 1, rice: 1, soft: 1
    },
  ...
  {
    consumption : {
      alcohol: -2, bread: 2, cheese: 1, fish: 1, harvestExotic: 1,
      harvestLocal: 1, meatRed: 1, meatWhite: 1, rice: 1, soft: 1
    },
    expectedResult: { emission: -1, waste: -1 },
...
const SAMPLE_DATA = [
  {
    consumption : {
      alcohol: 0, bread: 0, cheese: 0, fish: 0, harvestExotic: 0,
      harvestLocal: 0, meatRed: 0, meatWhite: 0, rice: 0, soft: 0
    },
  ...
  {
    consumption : {
      alcohol: 6, bread: 10, cheese: 5, fish: 4, harvestExotic: 5,
      harvestLocal: 8, meatRed: 6, meatWhite: 3, rice: 6, soft: 13
    },
  ...
  {
    consumption : {
      alcohol: 7, bread: 7, cheese: 7, fish: 7, harvestExotic: 7,
      harvestLocal: 7, meatRed: 7, meatWhite: 7, rice: 7, soft: 7
    },
